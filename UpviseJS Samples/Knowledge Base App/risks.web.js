...
function writeRisks(risks) {
    ...
    for (var i = 0; i < risks.length; i++) {
        var risk = risks[i];
        var onclick = User.isAdmin() ? "editRisk({risk.id})" : "";
        List.add([risk.name, Format.options(risk.probability, Qhse.RISK_OPTIONS), Format.options(risk.severity, Qhse.RISK_OPTIONS), risk.type], onclick);
    }
}
...
Qhse.writeRiskMatrix = function () {
    ...
    for (var i = 0; i < risks.length; i++) {
        var risk = risks[i];
        var key = risk.severity + ":" + risk.probability;
        map.increment(key, 1);
    }
...
Qhse.onRisks = function (severity, probability) {
    Toolbar.setTitle("")
